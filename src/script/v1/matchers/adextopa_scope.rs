...
mod tests {
  ...
  fn it_works1() {
    ...
    if let Ok(MatcherSuccess::Token(token)) = result {
      ...
      assert_eq!(*first.get_value_range(), SourceRange::new(19, 40));
      assert_eq!(*first.get_raw_range(), SourceRange::new(19, 40));
      assert_eq!(first.value(), "# Just a test comment");
      assert_eq!(first.raw_value(), "# Just a test comment");
      ...
      assert_eq!(*third.get_value_range(), SourceRange::new(60, 70));
      assert_eq!(*third.get_raw_range(), SourceRange::new(60, 70));
      assert_eq!(third.value(), "test2=test");
      assert_eq!(third.raw_value(), "test2=test");

      let forth = scope.get_children()[3].borrow();
      assert_eq!(forth.get_name(), "Comment");
      assert_eq!(*forth.get_value_range(), SourceRange::new(70, 86));
      assert_eq!(*forth.get_raw_range(), SourceRange::new(70, 86));
